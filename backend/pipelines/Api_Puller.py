...
def run_pipeline_for_user(user_id,refresh_token,prefix_valid,prefix_invalid,session_id):
    ...
    (
        ...
        | 'Write Valid To GCS' >> WriteToText(file_path_prefix=f'gs://{valid_blob_path}',file_name_suffix='.csv',shard_name_template='',header=','.join(columns))
    )
    (
        invalid_records
        | 'Invalid To CSV' >> beam.Map(lambda r: f'{r["error"]},"{json.dumps(r["record"])}"')
        | 'Write Invalid To GCS' >> WriteToText(file_path_prefix=f'gs://{invalid_blob_path}',file_name_suffix='.csv',shard_name_template='',header='error,record_json')
    )
    return p.run()
