...
def upsample_underrepresented(df: pd.DataFrame, col: str) -> pd.DataFrame:
    ...
    total       = len(df)
    target_min  = int(total * UNDERREPRESENTED_THRESHOLD)
    frames      = [df]
    ...
    for label, count in counts.items():
        if count < target_min:
            ...
            frames.append(extra)
            logger.info(
                f"Upsampled '{label}' in '{col}': {count} → {count + needed} rows"
            )
...
def downsample_dominant(df: pd.DataFrame, col: str) -> pd.DataFrame:
    counts      = df[col].fillna("Unknown").value_counts()
    total       = len(df)
    target_max  = int(total * DOMINANCE_THRESHOLD)
    frames      = []
    ...
    for label, count in counts.items():
        ...
        if count > target_max:
            ...
            logger.info(
                f"Downsampled '{label}' in '{col}': {count} → {target_max} rows"
            )
