...
All GCP calls are mocked. No Cloud Function entry point exists;
the ML pipeline is triggered via server.py /ml endpoint.
"""
...
class TestRunMlSession:
    ...
    def test_calls_preprocessing(
        self, mock_preprocess, mock_init, mock_batch,
        mock_update, mock_thread
    ):
    ...
    @patch("ml.ml_trigger.initialize_session")
    @patch("ml.ml_trigger._run_ml_preprocessing")
    def test_handles_exception_gracefully(self, mock_preprocess, mock_init):
        from ml.ml_trigger import _run_ml_session
    ...
    @patch("ml.ml_trigger.initialize_session")
    @patch("ml.ml_trigger._run_ml_preprocessing")
    def test_handles_init_exception_gracefully(self, mock_preprocess, mock_init):
        from ml.ml_trigger import _run_ml_session
